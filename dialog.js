...
function dialog(id, icon, closefor) {
	...
	if (Cookies.get(id + 'closed') != "closed") {
		dialogs = dialogs + 1;
		$("#" + id.toString()).html('<div style="position: relative;text-align: right;top: -16px;right:10px;width:auto;height:auto;margin:0;padding:0;background:0;cursor:pointer;" onclick="$(\'#' + id + '\').hide();'+Cookies.set(\'' + id + 'closed\', \'closed\', {expiry : 604800})'">X</div><img src="'+ icon + '" style="vertical-align:middle;"/>&nbsp;&nbsp;&nbsp;<span class="txt">' + $("#" + id.toString()).html()) + '</span>';
